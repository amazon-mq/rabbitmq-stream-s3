...
#
#   pre-4.4-defaults   the defaults the plugin shipped before the read path was
#                      retuned, on a stream whose fragment count puts group
#                      nodes in the manifest.
#   near-client-cap    a tuned configuration pushed until the consumer's own
#                      connection is the limit.
#
# Both profiles are what the brokers behind the measured column really ran, so
# neither is what ships today. That is the point: this script checks the harness
# against real measurements, and the measurements predate the new defaults. Use
# `s3-bench-defaults.sh` to measure the shipped configuration.
#
...
SCENARIOS=(
  "pre-4.4-defaults  4  64   8   64 grouped 0   249.1"
