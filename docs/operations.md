...
Read `committed_bytes` against `fetch_ceiling_bytes` for the fetch side, and
`buffered_bytes` plus `committed_bytes` against `memory_ceiling_bytes` for the
memory side. The memory bound is on the sum rather than on the buffer alone, so
reading `buffered_bytes` against the ceiling on its own reports headroom that is
not there. `prefetch_stall_fetch_budget` and `prefetch_stall_buffer` say which
of the two a reader is actually held by.
