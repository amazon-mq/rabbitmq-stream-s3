...
New ranges are issued at the fetch frontier while a byte budget and a depth cap allow. `prefetch_window_max` (128 MiB) is partitioned between the two things a reader holds bytes for: fetching reserves a share of it, and buffering gets the rest. `prefetch_max_depth` (64) is the ceiling on requests in flight, not the operating point.
...
Fetching's share is a floor rather than a cap: it takes whatever the buffer is not using, and only falls back to the reserved share once the buffer has taken the space. Below that share it does not consult the buffer at all, so a full buffer can slow a reader down but never stop it issuing - every delivered byte moves from committed to buffered and stays there until the consumer reads it, so a shared budget would let the buffer lock the fetch side out entirely.

A consumer that stops reading needs no separate throttle: the reader holds what it has spoken for and stops. That bound is twice `prefetch_window_max`, not `prefetch_window_max` itself, because the fetch share is a guarantee the buffer cannot take - the worst case is a full share committed on top of a buffer holding the rest. **`prefetch_window_max` therefore describes about half a reader's memory, not all of it.**
