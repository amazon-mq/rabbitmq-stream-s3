...
Fetching does not consult the buffer at all, so a full buffer can never stop a reader issuing: every delivered byte moves from committed to buffered and stays there until the consumer reads it, so a bound the two shared would let the buffer lock the fetch side out entirely.
