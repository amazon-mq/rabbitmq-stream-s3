...
New ranges are issued at the fetch frontier while two budgets and a depth cap allow. The budgets answer different questions and are bounded separately: how hard to fetch, which is the concurrency target's worth of bytes, and how far ahead to buffer, which is the *prefetch window*. `prefetch_max_depth` (64) is the ceiling on requests in flight, not the operating point.
...
The window is the buffer budget and moves on its own signal. It starts at one request and:

- doubles on a buffer miss, capped at `prefetch_window_max` (128 MiB) - a miss means the reader is not fetching far enough ahead, so it is a signal to fetch more, not to back off
- gives one request back after a window's worth of bytes has been served without a miss

Only the first miss for a given read counts: the serve attempt re-runs on every delivery while a read waits, and counting each of those would run the window to its ceiling on one slow read.

A consumer that stops reading needs no separate throttle - the buffer fills to the window and no further requests are issued. The window is not the whole of the memory bound, though, because it gates issuing a range rather than holding bytes: the ranges already committed when the gate closes still deliver, and their bytes land in a buffer behind it. Worst case per reader is therefore the sum of the two budgets - `prefetch_window_max` plus `prefetch_max_depth` requests' worth - rather than either alone.
...
The read in hand lifts both budgets. They govern *prefetch*, but a read of N bytes cannot be served while fewer than N are outstanding, so each ceiling is floored at whatever the pending read needs. Reads are chunk sized, so this only bites when a chunk is larger than the window. Refusing to fetch instead would not bound anything - it would simply never serve the read: at the ceiling the window cannot grow, so nothing further is ever requested, and the read deadline only refetches to the same ceiling and stalls again.
