...
The concurrency target is searched for rather than configured, because the answer is not a property of the configuration: the same reader wants materially different concurrency against a store with a round trip to hide behind and one without, and against a cold connection pool and a warm one. A reader starts at one request and the search doubles it every sample the measured fetch rate keeps answering, then hill-climbs a step at a time around the peak and comes back down when the rate falls away. Starting at one request is also what keeps a reader's attach cost proportional to demand: a consumer that reads one record and disconnects never pays for concurrency it did not use. `prefetch_auto_tune = false` pins the target at `prefetch_inflight_initial` for a reader's whole life, which is the way back and what to set for a reproducible throughput measurement.

The fetch ceiling does not consult the buffer at all, so a full buffer can never lock the fetch side out: every delivered byte moves from committed to buffered and stays there until the consumer reads it, so a ceiling the two shared would hand the whole budget to the buffer over time. A full buffer can still stop a reader issuing, through the memory bound below, which is the `prefetch_stall_buffer` counter's job to report. What it cannot do is squeeze fetching down to nothing.

A consumer that stops reading needs no separate throttle: the reader holds what it has spoken for and stops. `prefetch_max_memory` is that bound, reached when a full fetch ceiling is committed on top of a buffer holding as much - which is why fetching gets half of it rather than all.
...
The read in hand lifts both bounds. They govern *prefetch*, but a read of N bytes cannot be served while fewer than N are outstanding, so each is floored at whatever the pending read needs. Reads are chunk sized, so this only bites when a chunk is larger than the budget. Refusing to fetch instead would not bound anything - it would simply never serve the read, and the deadline only refetches to the same ceiling and stalls again.

S3 errors drive a separate exponential retry-delay backoff, and `pool_busy` (the connection pool saturated) drives a third, milder one. Both grow once per retry round rather than once per failed range: a reset connection fails every pipelined range at once, and one fault must cost one doubling.
...
Once every byte of the current fragment has been asked for, the fetch frontier spills into the next one rather than waiting for the current fragment to arrive first, so the budgets stay spent across a boundary instead of draining at every fragment. The walk continues into the fragment after that as each one fills, up to `prefetch_max_depth` fragments ahead, a backstop the byte budget reaches first for any fragment of a normal size: stopping at the first fully-spoken-for fragment would cap reach at a fragment tail to one fragment however large the budgets are. Each fragment ahead of the current one holds its own buffer, and a fragment is only ever looked ahead to in order to put a range in it.
...
A group fetch that fails transiently is memoised too, as *failed* rather than as an answer, and the look-ahead arms the retry backoff itself. Nothing else on that path would: the ranges already queued are healthy, so no request error runs. The memo carries which clock to wait on, and whether to attempt again is read off that clock rather than stored beside it, so the memo is honoured while it is armed and re-attempted once it is not.

Which clock depends on what failed. A store error earns the fault clock, which grows from 1s to 30s, because something has to pace attempts that each cost a synchronous GET inside the reader. A descent that could not get a pooled connection never reached the store, so it earns the pool clock instead, which runs 25-500ms: the object was never asked for, and what the reader is waiting for is a connection rather than a store that is failing. To make that cheap enough to retry often, the look-ahead's group fetches use a short checkout and no retries of their own, unlike offset resolution, which builds the same iterator but is the consumer's read being set up and has nothing to do but wait.

The ranges in flight are left alone when this happens: the failure is in advancing the iterator, which says nothing about the fragment GETs already on the wire, and cancelling one closes its pooled connection.
