...
handle_call(
    ...
    _From,
    #?MODULE{
        max_size = MaxSize, monitors = Monitors, available = Available, checkouts = Checkouts
    } = State
) ->
    %% Answered from the current state rather than from what `grow/1` last
    %% decided: a connection may have died since, and the caller is asking about
    %% now.
    %%
    %% `monitors` counts a connection from `open` onwards, so a pool still
    %% handshaking its way up to `max_size` would answer `true` on the count
    %% alone. The caller reads that as the pool having grown as far as it can
    %% and cuts its concurrency for it, which is the cost of growing rather than
    %% evidence of having grown too far. A connection that is neither available
    %% nor checked out is still coming up.
    Opening = map_size(Monitors) - length(Available) - map_size(Checkouts),
    {reply, map_size(Monitors) >= MaxSize andalso Opening =< 0, State};
