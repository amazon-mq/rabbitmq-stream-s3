...
pool_busy_short_circuits_the_rest_of_a_batch_test() ->
    ...
    %% Reads that cannot be served fill the reader to its pinned concurrency;
    %% failing every range then leaves a batch for the retry to re-issue, which
    %% is when a saturated pool costs the most.
    Core1 = lists:foldl(
        fun(_, Acc) ->
