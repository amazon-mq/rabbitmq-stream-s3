...
`run.sh` runs one test against the already-up cluster and, unlike `ci/run-jepsen.sh`, leaves it running so you can iterate and inspect `store/`. It reads `FAULTS`, `FINAL_READ_TIER`, `TIME_LIMIT`, `RATE` and `CONCURRENCY` from the environment and passes any extra arguments through to `lein run test`:

```sh
FAULTS=s3-outage,leader-move ./run.sh
FAULTS=leader-move FINAL_READ_TIER=local TIME_LIMIT=300 ./run.sh
```
...
The storage-tier nemeses make data really tier to S3 and exercise the read-from-S3 path under fault. Small segments and fragments plus padded payloads force segments to roll, upload and trim within a short run; MinIO needs a KMS key for the plugin's SSE uploads (see `docker/docker-compose.yml`). The `--faults` option selects `s3-outage` and `s3-latency` (via Toxiproxy), `leader-move` (each tick, transfer a random stream's leader to a replica, relocating that writer and bumping the stream epoch so a deposed writer's in-flight upload must be fenced), and `member-churn` (each tick, `delete_replica` a random replica from a random stream so an osiris member departs its node for good mid-sync; bounded to keep every stream at leader + at least one replica). Reads-from-S3 coverage is a separate knob rather than a fault: `--final-read-tier s3` (the `run.sh` default) forces one `evaluate_local_retention` after faults heal so the final verification reads drain from the remote tier. It forces no trimming the plugin would not do on its own — the plugin trims uploaded segments eagerly on every manifest advance — it only makes the tier the final reads hit deterministic rather than timing-dependent; `--final-read-tier local` leaves the reads on the local tier.

Three extra checkers run alongside the kafka anomaly checkers. The `:tiering` coverage checker scrapes the plugin's S3 counters before teardown and fails the run unless fragments were uploaded, and with `--final-read-tier s3` that reads were served from S3, and with `leader-move` that the epoch advanced; this keeps a run from passing green while silently not using the remote tier or not moving leaders. The `s3-outage` fault is instead proven injected by the Toxiproxy status check in the nemesis (it fails on a non-2xx response), since the plugin tolerates an outage gracefully and need not surface an error counter. The `:durability` checker reads every stream end-to-end after the run and fails if any acknowledged write is lost or duplicated; it is authoritative under `leader-move`, where the kafka offset analyzers are downgraded to advisory (see "Why the Kafka workload").
...
A run under `partition,s3-outage,s3-latency` with the final reads forced off the local tier stays `:valid? true` while serving thousands of reads from the remote tier:

```sh
FAULTS=partition,s3-outage,s3-latency FINAL_READ_TIER=s3 TIME_LIMIT=150 RATE=50 CONCURRENCY=10 ./run.sh
```

A run under `leader-move` stays `:valid? true` with the durability checker confirming zero loss and zero duplication across epochs in the double digits:

```sh
FAULTS=leader-move TIME_LIMIT=150 RATE=50 CONCURRENCY=10 ./run.sh
```
